...
The current repository-layout directory contains development-review artifacts only. It is not a Central upload bundle because its version ends in SNAPSHOT and it has no Yanbor PGP signatures. Licensed production customers should continue using an authenticated private Maven repository.
